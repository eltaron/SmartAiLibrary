...
from services.rec_service.models.ncf import NeuralCF, InteractionDataset, save_model
from shared.config import settings
from training.id_mapping import build_item_mapping, build_user_mapping, save_mappings
...
def load_interactions(data_path: str) -> tuple[list[int], list[int], list[int], dict[str, int], dict[str, int]]:
    """
    Load interactions from JSON file.

    Expected format: [{"user_id": str|int, "isbn": str, "label": int}, ...]
    """
    with open(data_path, encoding="utf-8") as f:
        data = json.load(f)

    user_keys = [str(d["user_id"]) for d in data]
    isbns = [d["isbn"] for d in data]
    labels = [d["label"] for d in data]

    user_map = build_user_mapping(user_keys)
    item_map = build_item_mapping(isbns)

    user_ids = [user_map[uid] for uid in user_keys]
    item_ids = [item_map[isbn] for isbn in isbns]

    return user_ids, item_ids, labels, user_map, item_map
...
def main():
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default=settings.NCF_MODEL_PATH,
        help="Output model path",
    )
    parser.add_argument(
        "--mappings-path",
        type=str,
        default=settings.NCF_MAPPINGS_PATH,
        help="Output mappings path",
    )
    ...
    user_ids, item_ids, labels, user_map, item_map = load_interactions(args.data_path)
    ...
    save_mappings(args.mappings_path, user_map, item_map)
    ...
    if MLFLOW_AVAILABLE:
        ...
        mlflow.log_artifact(args.mappings_path)
