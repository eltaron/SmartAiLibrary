...
class Settings(BaseSettings):
    ...
    NCF_MODEL_PATH: str = Field(
        default="/models/ncf_weights.pt",
        description="Path to NCF model weights",
    )
    NCF_MAPPINGS_PATH: str = Field(
        default="/models/ncf_mappings.json",
        description="Path to NCF user/item ID mappings",
    )
    PINECONE_GLOBAL_NAMESPACE: str = Field(
        default="",
        description="Pinecone namespace for global chunk/synopsis search",
    )
    PINECONE_SYNOPSES_NAMESPACE: str = Field(
        default="synopses",
        description="Pinecone namespace for book synopsis vectors (CBF)",
    )
    ...
    RAG_LLM_MODEL: str = Field(
        ...
        description="LLM model for RAG Q&A (Groq)",
    )
    RAG_LLM_BASE_URL: str = Field(
        default="https://api.groq.com/openai/v1",
        description="OpenAI-compatible base URL for RAG LLM",
    )
