...
- API keys: [Pinecone](https://www.pinecone.io/) (required), [Groq](https://console.groq.com/) (required for RAG)
- Trained NCF model weights (optional — cold-start fallback used if missing; see [Models folder](#models-folder) below)
...
| `GROQ_API_KEY` | Groq API key (for RAG service — get one free at [console.groq.com](https://console.groq.com/)) |
...
| `PINECONE_GLOBAL_NAMESPACE` | `""` | Pinecone namespace for global chunk search |
| `PINECONE_SYNOPSES_NAMESPACE` | `synopses` | Pinecone namespace for book synopsis vectors |
| `LOG_LEVEL` | `INFO` | Logging level |
| `CHUNK_SIZE` | 512 | Tokens per chunk |
| `EMBEDDING_MODEL` | `sentence-transformers/all-mpnet-base-v2` | Embedding model |
| `RAG_LLM_MODEL` | `llama-3.3-70b-versatile` | LLM for RAG (via Groq) |
| `RAG_LLM_BASE_URL` | `https://api.groq.com/openai/v1` | OpenAI-compatible endpoint for RAG LLM |

See `shared/config.py` for the full list of 40+ configuration options.

## Models folder

The `models/` directory (at project root) is mounted read-only into the `rec-service` container at `/models`.

| File | Purpose |
|------|---------|
| `models/ncf_weights.pt` | Trained NCF model weights (PyTorch) |
| `models/ncf_mappings.json` | User/item ID mappings produced during training |

**If these files are missing** the service starts normally but falls back to cold-start (CBF-only) recommendations. To train and generate them:

```bash
# Train NCF model — saves weights + mappings to models/
python -m training.ncf_train

# Or run the monthly retrain pipeline
python -m training.pipelines.monthly_ncf_retrain
```

The `summarise-service` downloads `google/flan-t5-large` from HuggingFace on first startup (~1 GB). To pre-cache it inside the image, run training pipelines or mount a local HuggingFace cache via:

```yaml
# in infra/docker-compose.yml under summarise-service volumes:
