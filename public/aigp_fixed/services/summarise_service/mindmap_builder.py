...
class MindMapBuilder:
    ...
    def build(self, book_summary: str, book_title: str) -> dict:
        ...
        try:
            raw = self._summariser.model.generate(
                **self._summariser.tokenizer(
                    prompt,
                    return_tensors="pt",
                    max_length=1024,
                    truncation=True,
                ).to(self._summariser.device),
                max_new_tokens=512,
                num_beams=2,
            )

            text = self._summariser.tokenizer.decode(raw[0], skip_special_tokens=True)
