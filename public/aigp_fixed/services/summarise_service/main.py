...
def check_gpu_queue(redis: RedisClient) -> bool:
...
async def summarise(
    ...
    try:
        ...
        if req.include_mindmap:
            ...
            mindmap = mindmap_builder.build(summary, req.isbn)
            response_data["mindmap"] = mindmap

        import orjson

        await redis.setex(cache_key, settings.REDIS_TTL_SUMMARY, orjson.dumps(response_data))
