...
class TestContentBasedFilter:
    ...
    def test_similar_books_returns_correct_count(self, mock_pc):
        ...
        mock_pc.query.return_value = [
            {"values": [0.1] * 768, "metadata": {"isbn": "isbn-1", "title": "Book 1"}},
            {"values": [0.2] * 768, "metadata": {"isbn": "isbn-2", "title": "Book 2"}},
            {"values": [0.3] * 768, "metadata": {"isbn": "isbn-3", "title": "Book 3"}},
        ]
        ...
        results = cbf.similar_books("seed-isbn", top_k=3)

        assert len(results) == 3

    @patch("services.rec_service.models.cbf.pinecone_client")
    def test_seed_isbn_not_in_results(self, mock_pc):
        ...
        cbf = ContentBasedFilter()
        results = cbf.similar_books("seed-isbn", top_k=5)

        isbns = [r["isbn"] for r in results]
        assert "seed-isbn" not in isbns

    @patch("services.rec_service.models.cbf.pinecone_client")
    def test_empty_results_when_seed_not_found(self, mock_pc):
        ...
        cbf = ContentBasedFilter()
        results = cbf.similar_books("nonexistent-isbn")

        assert results == []

    @patch("services.rec_service.models.cbf.pinecone_client")
    def test_similar_to_query(self, mock_pc):
        ...
        results = cbf.similar_to_query(query_vec, top_k=5)
...
class TestBatchSimilarBooks:
    ...
    @patch("services.rec_service.models.cbf.pinecone_client")
    def test_batch_empty_input(self, mock_pc):
        """Test batch_similar_books with empty input."""
        from services.rec_service.models.cbf import ContentBasedFilter

        cbf = ContentBasedFilter()
        results = cbf.batch_similar_books([], top_k=10)

        assert results == []

    @patch("services.rec_service.models.cbf.pinecone_client")
    def test_batch_multiple_seeds(self, mock_pc):
        """Test batch_similar_books with multiple seed ISBNs."""
        from services.rec_service.models.cbf import ContentBasedFilter

        mock_pc.query.side_effect = [
            [{"values": [0.1] * 768, "metadata": {"isbn": "isbn-1"}}],
            [{"values": [0.2] * 768, "metadata": {"isbn": "isbn-2"}}],
        ]

        cbf = ContentBasedFilter()
        results = cbf.batch_similar_books(["isbn-1", "isbn-2"], top_k=5)
...
class TestGenreCentroid:
    ...
    @patch("services.rec_service.models.cbf.pinecone_client")
    def test_genre_centroid_shape(self, mock_pc):
        ...
        cbf = ContentBasedFilter()
        centroid = cbf.get_genre_centroid("science-fiction")

        assert centroid.shape == (768,)

    @patch("services.rec_service.models.cbf.pinecone_client")
    def test_genre_not_found_returns_zeros(self, mock_pc):
        ...
        cbf = ContentBasedFilter()
        centroid = cbf.get_genre_centroid("nonexistent-genre")
